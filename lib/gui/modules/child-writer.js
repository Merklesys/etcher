...
ipc.connectTo(IPC_SERVER_ID, () => {
  ...
  ipc.of[IPC_SERVER_ID].on('write', (options) => {
    ...
    const onFinish = (result) => {
      ...
      writers.forEach((writer) => {
        completed = completed && (writer.finished || writer.hadError)
      })
    ...
    const onError = function(error) {
      log(`Error: ${result.drive.device} – ${error.message}`)
  ...
  ipc.of[IPC_SERVER_ID].on('connect', () => {
    ...
    // TODO: node-ipc drops this message consistently;
    // either find the bug & patch it, or find an alternative
    // NOTE: If .emit('log') it works; other msg names like 'asd' do not
